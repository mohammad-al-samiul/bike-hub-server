...
export const initiatePayment = async (paymentData: TPaymentProps) => {
  try {
    const response = await axios.post(config.payment_url as string, {
      ...
      success_url: `${config.frontend_url}/api/confirmation?transactionId=${paymentData.transactionId}&status=success`,
      fail_url: `${config.frontend_url}/api/confirmation?status=failed`,
